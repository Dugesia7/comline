...
void function(int stage)
{
	...
	for (d = 0; d < 4; d++)
	{
		...
		//	CAUTION v. NOT reach the coordinate you have reached previously
		if (coordinate_record[location_now[0] + 10][location_now[1] + 10] == true && stage != scN - 1)
		{
			//	printf("%d\tCAUTION V.\n", stage);
			used_direction_amount[d]--;
			//	location back
		...
		function(stage + 1);
		if(location_now[0]||location_now[1])coordinate_record[location_now[0] + 10][location_now[1] + 10] = false;
